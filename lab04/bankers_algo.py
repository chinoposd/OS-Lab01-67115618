...
# System Resources:
# [GPU_Type_A, GPU_Type_B, High_Speed_RAM(TB)]
total_resources = np.array([10, 5, 7])

# Max requirement for each ML Job
max_need = np.array([
    [7, 5, 3],
    [3, 2, 2],
    [9, 0, 2]
...
def main():
    ...
    available = total_resources - np.sum(
        allocated,
        axis=0
    )
    ...
    if safe:
        ...
        print(
            f">> SYSTEM IS SAFE. Execution Sequence: "
            f"{' -> '.join(sequence)}"
        )
        ...
        print(
            ">> WARNING: SYSTEM IS UNSAFE! "
            "Granting locks will cause a Deadlock."
        )

        print(
            ">> OS Scheduler denies the request "
            "and forces the process to wait."
        )
